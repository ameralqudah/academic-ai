...
للزائر العادي يعيد فقط `{"status":"ok"}` (أو `503` إن كانت قاعدة البيانات غير متاحة) — التقرير
المفصّل لم يعد عامًّا. **سجّل الدخول بحساب المدير أولًا** ثم افتح الرابط لترى التقرير الكامل.
