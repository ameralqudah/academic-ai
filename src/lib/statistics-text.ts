...
export const ASSIGNMENT = new RegExp(String.raw`(?<![\p{L}\p{N}])${SYMBOL}\s*(?:\(\s*[\d.,\s]+\))?\s*[=<>≤≥]\s*${NUMBER}`, 'gu');
/** Any decimal number (dot or comma), and percentages. */
export const DECIMAL = new RegExp(String.raw`(?<![\p{L}\p{N}])(?:\d+[.,]\d+|[.,]\d+)(?![\p{N}])|\d+(?:[.,]\d+)?\s?%`, 'gu');
