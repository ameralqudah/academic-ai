...
/** Text that came from a user, made safe to place inside HTML. */
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #dce3ec;border-radius:12px;">
      <tr>
        <td style="padding:32px;text-align:${align};">
          <p style="margin:0 0 16px;font-size:16px;font-weight:600;">${escapeHtml(copy.greeting(input.name))}</p>
...
const VERIFY_COPY = {
  ar: {
    subject: 'تأكيد بريدك الإلكتروني — المساعد الأكاديمي للبحث العلمي',
    greeting: (name: string | null) => (name ? `مرحبًا ${name}،` : 'مرحبًا،'),
    body: 'أكّد أن هذا البريد الإلكتروني لك بالضغط على الزر أدناه.',
    action: 'تأكيد البريد الإلكتروني',
    expiry: (hours: number) => `الرابط صالح لمدة ${hours} ساعة.`,
    ignore: 'إن لم تنشئ حسابًا، تجاهل هذه الرسالة.',
    fallback: 'إن لم يعمل الزر، انسخ هذا الرابط والصقه في المتصفح:',
  },
  en: {
    subject: 'Confirm your email — Academic AI Research Assistant',
    greeting: (name: string | null) => (name ? `Hi ${name},` : 'Hi,'),
    body: 'Confirm that this email address is yours by using the button below.',
    action: 'Confirm email',
    expiry: (hours: number) => `This link is valid for ${hours} hours.`,
    ignore: "If you didn't create an account, ignore this email.",
    fallback: "If the button doesn't work, copy this link into your browser:",
  },
} as const;

export function emailVerificationEmail(input: VerifyInput): EmailMessage {
  const copy = VERIFY_COPY[input.locale];
  const dir = input.locale === 'ar' ? 'rtl' : 'ltr';
  const align = input.locale === 'ar' ? 'right' : 'left';

  const text = [copy.greeting(input.name), '', copy.body, '', input.url, '', copy.expiry(input.expiresHours), copy.ignore].join('\n');
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #dce3ec;border-radius:12px;">
      <tr>
        <td style="padding:32px;text-align:${align};">
          ...
          <p style="margin:0 0 8px;font-size:13px;color:#5d6b7f;">${copy.expiry(input.expiresHours)}</p>
          <p style="margin:0 0 24px;font-size:13px;color:#5d6b7f;">${copy.ignore}</p>
          <p style="margin:0 0 6px;font-size:12px;color:#5d6b7f;">${copy.fallback}</p>
          <p style="margin:0;font-size:12px;word-break:break-all;" dir="ltr"><a href="${input.url}" style="color:#0f5c4a;">${input.url}</a></p>
        </td>
