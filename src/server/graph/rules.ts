/**
 * Edge rules: what each relation means for impact analysis (TARGET_ARCHITECTURE
 * §G.2, R6).
 *
...
export interface EdgeRule {
   ...
   * model and its elements): a change to a part is the same change to the
   * whole, so the container passes on the kind of change it received.
   */
  container?: boolean;
...
export const EDGE_RULES: readonly EdgeRule[] = [
  ...
  { rel: 'executes', from: ['analysis_run'], to: ['analysis'], dependency: true, downstream: REVIEW_INVALIDATE, reason: 'The run executed this version of the analysis.' },
  { rel: 'uses_data', from: ['analysis_run'], to: ['dataset_version'], dependency: true, downstream: REVIEW_INVALIDATE, reason: 'The run used this dataset version.' },
  { rel: 'produced_by', from: RESULTS, to: ['analysis_run'], dependency: true, downstream: REVIEW_INVALIDATE, reason: 'Produced by this run.' },
  { rel: 'tests', from: ['result_value'], to: ['hypothesis'], dependency: true, downstream: REVIEW_INVALIDATE, propagate: false, reason: 'This value decides the hypothesis.' },
  ...
  // Literature
  { rel: 'extracted_from', from: ['evidence'], to: ['source'], dependency: true, downstream: REVIEW_INVALIDATE, reason: 'The evidence was extracted from this source.' },
  { rel: 'supports', from: ['evidence'], to: ['hypothesis', 'construct', 'citation'], dependency: false, reason: 'Supporting evidence.' },
  { rel: 'contradicts', from: ['evidence'], to: ['hypothesis', 'construct', 'citation'], dependency: false, reason: 'Contradicting evidence.' },
  { rel: 'of_source', from: ['citation'], to: ['source'], dependency: true, downstream: REVIEW_INVALIDATE, reason: 'The citation refers to this source.' },
  { rel: 'about', from: ['citation'], to: ['construct', 'hypothesis'], dependency: true, downstream: REVIEW, reason: 'The cited claim is about this.' },

  // Manuscript
  { rel: 'reports', from: ['block'], to: [...RESULTS], dependency: true, downstream: REVIEW_INVALIDATE, reason: 'The text reports this result.' },
  { rel: 'cites', from: ['block'], to: ['citation'], dependency: true, downstream: REVIEW_INVALIDATE, reason: 'The text cites this.' },
  { rel: 'describes', from: ['block'], to: ['construct', 'hypothesis', 'analysis', 'instrument', 'instrument_item', 'conceptual_model', 'dataset_version'], dependency: true, downstream: REVIEW, reason: 'The text describes this.' },
  { rel: 'summarizes', from: ['block'], to: ['section'], dependency: true, downstream: REVIEW, reason: 'The text summarises this section.' },
  { rel: 'refers_to', from: ['block'], to: ['section', 'block', ...RESULTS], dependency: true, downstream: REVIEW, reason: 'The text cross-references this.' },
  { rel: 'part_of', from: ['block', 'section'], to: ['section', 'manuscript'], dependency: false, reason: 'Structure.' },
  ...
  { rel: 'supersedes', from: NODE_TYPES, to: NODE_TYPES, dependency: false, reason: 'Replaces an earlier object of the same type.' },
 ...
 * change it, that change runs its own impact analysis. Two exceptions: a
 * container passes on what its part received, and replacing a node
 * (`reviewPropagates`) asks for review all the way down — every block that
 * reports a superseded run should say so.
 */
export function propagatedKind(
  severity: Severity,
  incoming: ChangeKind,
  rule: EdgeRule,
  reviewPropagates = false,
): ChangeKind | null {
  if (rule.propagate === false) return null;
  if (rule.container) return incoming;
  if (severity === 'invalidates') return 'structural';
  if (severity === 'review' && reviewPropagates) return 'substantive';
  return null;
}
